...
class ClientCategoryModule extends VuexModule {
    category: ICategory[] = [];
    ...
    async fetchAll(): Promise<{ category: ICategory[]; categoryLoaded: boolean }> {
        const category = await store.$repository.categoryClient.fetchAll();
...
export const ClientCategoryStore = getModule(ClientCategoryModule);
