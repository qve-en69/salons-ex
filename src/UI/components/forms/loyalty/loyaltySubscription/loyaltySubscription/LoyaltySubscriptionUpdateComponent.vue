        ...
        <a-form-model>
            ...
            <div class="form-input">
                ...
                <a-form-model-item :prop="form.phone">
                    <a-input v-model="form.phone" placeholder="+7 (___) ___-__-__" v-mask="['+7 (###) ###-##-##']" />
                </a-form-model-item>
            </div>
            <FormGroup v-slot="attrs" :form="form" field="abonement_type_id" label="Тип абонемента" show-custom-error>
                <a-select
                    ...
                    @input="attrs.change"
                >
                    <a-select-option v-for="item in form.types" :key="item.id">
                        {{ item.name }}
                    </a-select-option>
                </a-select>
            </FormGroup>
...
export default class LoyaltySubscriptionUpdateFormComponent extends Vue {
    @Prop() form!: LoyaltySubscriptionUpdateForm;
}
</script>
